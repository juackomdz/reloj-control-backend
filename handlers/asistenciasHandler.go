...
func CheckOut(c echo.Context) error {
	...
	exito := database.Conectar().Model(&models.RegistroAsistencias{}).Where(&models.RegistroAsistencias{UsuariosID: registro.UsuariosID}).Where("hora_salida is null").Update("hora_salida", &ahora)
